        ...
        <div className="ud-main">
          ...
          <div>
            <div className="ud-section-header">
              ...
              <button className="ud-view-all" onClick={() => navigate('/appointments')}>View all →</button>
            </div>
            {upcomingAppointments.length === 0 ? (
              <div className="ud-empty">
                <div className="ud-empty-icon">📅</div>
                <p>No upcoming appointments. <button className="ud-view-all" onClick={() => navigate('/book')}>Book one now →</button></p>
              </div>
            ) : (
              <div className="ud-appt-card">
                {upcomingAppointments.map((appt, idx) => {
                  const statusColor = statusColors[appt.status] || { bg: '#f3f0eb', color: '#6b5a4e' };
                  return (
                    <div key={idx} className="ud-appt-row">
                      <div className="ud-appt-avatar">
                        {serviceEmoji[appt.service] || '🤲🏻'}
                      </div>
                      <div className="ud-appt-info">
                        <div className="ud-appt-specialist">{appt.specialist}</div>
                        <div className="ud-appt-meta">
                          {appt.service} • {appt.date} at {appt.time}
                        </div>
                      </div>
                      <div className="ud-appt-status" style={{ background: statusColor.bg, color: statusColor.color }}>
                        {appt.status}
                      </div>
                    </div>
                  );
          ...
          <div>
            <div className="ud-reviews-header">
              <h2 className="ud-reviews-title">⭐ Specialist Reviews</h2>
              <div className="ud-reviews-stats">
                <div className="ud-reviews-stat">
                  <div className="ud-reviews-stat-value">
                    {averageRating}
                    <span style={{ fontSize: '14px', color: '#a8956b', marginLeft: '3px' }}>⭐</span>
                  </div>
                  <div className="ud-reviews-stat-label">Average Rating</div>
                </div>
                <div className="ud-reviews-stat">
            ...
            <div className="ud-reviews-container">
              ...
              <div className="ud-reviews-tabs">
                ...
                >
                  All
                  <span className="ud-tab-count">{reviews.length}</span>
                </button>
                {[5, 4, 3, 2, 1].map(rating => {
                  ...
                  return (
                    <button
                      key={rating}
                      className={`ud-reviews-tab${selectedRating === String(rating) ? ' active' : ''}`}
              ...
              {filteredReviews.length === 0 ? (
                <div className="ud-no-reviews">
                  ...
                  <div className="ud-no-reviews-subtext">Book an appointment to leave a review</div>
                </div>
              ) : (
                    ...
                    <div key={idx} className="ud-review-item">
                      <div className="ud-review-avatar-small">
                        {buildPhotoUrl(review.patientProfilePictureUrl) ? (
                          <img 
                            src={buildPhotoUrl(review.patientProfilePictureUrl)} 
                        ...
                        {!buildPhotoUrl(review.patientProfilePictureUrl) && (review.patientName?.charAt(0).toUpperCase() || '👤')}
                      </div>
                      <div className="ud-review-content">
                        <div className="ud-review-header">
                          <div className="ud-review-specialist">{review.patientName}</div>
                          <div className="ud-review-rating-stars">{'⭐'.repeat(review.rating)}</div>
                        </div>
                        <div className="ud-review-service"><i>Specialist:</i> {review.specialistName}</div>
                        <div className="ud-review-service"><i>Service:</i> {review.serviceName}</div>
                        {review.reviewText && <div className="ud-review-text">{review.reviewText}</div>}
